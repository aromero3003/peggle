...
bool leer_movimiento_inmovil(FILE *f, int16_t parametros[]) { return true; }
...
poligono_t *leer_geometria_circulo(FILE *f) {
    ...
    float radio = r;
    poligono_t *poligono = new poligono_t(radio, RESOL_BOLA_OBS);
...
poligono_t *leer_geometria_rectangulo(FILE *f) {
    ...
    float vertices[][2] = {{-ancho / 2, -alto / 2},
                           {ancho / 2, -alto / 2},
                           {ancho / 2, alto / 2},
                           {-ancho / 2, alto / 2}};
    ...
    putchar('\n');
    */
    poligono_t *poligono = new poligono_t(vertices, 4);
...
poligono_t *leer_geometria_poligono(FILE *f) {
    ...
    poligono_t *poligono = new poligono_t(parametros, puntos);
    if (poligono == NULL) return NULL;
