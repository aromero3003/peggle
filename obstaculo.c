...
obstaculo_t *obstaculo_levantar_obstaculo(FILE *f) {
    ...
    obstaculo_t *obstaculo = obstaculo_crear(poligono, color, mov, parametros, geo);
    if(obstaculo == NULL) {
...
void obstaculo_mover_circular(obstaculo_t *obstaculo, double dt) {
    obstaculo_rotar_centro(obstaculo, obstaculo->parametros[2] * dt * 60, obstaculo->parametros[0], obstaculo->parametros[1]);
}
...
bool obstaculo_dibujar(SDL_Renderer *renderer, obstaculo_t *obstaculo) {
    ...
    return obstaculo->dibujar ? poligono_dibujar(renderer, obstaculo->poligono) : true;
}
...
double obstaculo_distancia(const obstaculo_t *obstaculo, float xp, float yp, float *nor_x, float *nor_y) {
