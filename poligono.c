...
// puede
bool poligono_obtener_vertice(const poligono_t *poligono, size_t pos, float *x,
                              float *y) {
    if (poligono == NULL || poligono->n <= pos || poligono->vertices == NULL)
        return false;
...
bool poligono_agregar_vertice(poligono_t *poligono, float x, float y) {
    ...
    float(*aux)[2] = (float(*)[2])realloc(poligono->vertices,
                                          (poligono->n + 1) * sizeof(float[2]));
...
void poligono_rotar_centro(poligono_t *poligono, double rad, float centro_x,
                           float centro_y) {
...
void poligono_rotar_centro2(poligono_t *poligono, double rad, float centro_x,
                            float centro_y) {
    poligono_trasladar(poligono, -centro_x, -centro_y);
...
bool poligono_punto_dentro(poligono_t *poligono, float px, float py) {
    for (size_t i = 1; i < poligono->n - 1; i++) {
        if (punto_en_triangulo(
                px, py, poligono->vertices[0][0], poligono->vertices[0][1],
                poligono->vertices[i][0], poligono->vertices[i][1],
                poligono->vertices[i + 1][0],
                poligono->vertices[i + 1][1]) == true) {
            return true;
        }
    }
    return false;
...
void imprimir(poligono_t *poligono) {
    for (size_t i = 0; i < poligono->n; i++) {
        printf("  (%g ; %g) ", poligono->vertices[i][0],
...
bool poligono_dibujar(SDL_Renderer *renderer, poligono_t *poligono) {
    ...
    for (size_t i = 0; i + 1 < poligono->n; i++) {
        SDL_RenderDrawLine(
            renderer, poligono->vertices[i][0], poligono->vertices[i][1],
            poligono->vertices[i + 1][0], poligono->vertices[i + 1][1]);
    }
    SDL_RenderDrawLine(renderer, poligono->vertices[(poligono->n) - 1][0],
                       poligono->vertices[(poligono->n - 1)][1],
...
double poligono_distancia(const poligono_t *p, float xp, float yp, float *nor_x,
                          float *nor_y) {
    double d = 1 / 0.0;
    size_t idx = 0;
    ...
    for (size_t i = 0; i < p->n; i++) {
        float xi, yi;
        punto_mas_cercano(p->vertices[i][0], p->vertices[i][1],
                          p->vertices[(i + 1) % p->n][0],
                          p->vertices[(i + 1) % p->n][1], xp, yp, &xi, &yi);
